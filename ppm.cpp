...
ppm_t::ppm_t(ofstream *file, unsigned width, unsigned height) : m_file(file), m_width(width), m_height(height),
...
void ppm_t::set_background(color_t color) {
    for (uint32_t i = 0; i < m_resolution; ++i) {
...
void ppm_t::line(const color_t color, const uint32_t x1, const uint32_t y1, const uint32_t x2, const uint32_t y2) {
    double x = x1, y = y1;
    const double dx = x2 - x1;
    const double dy = y2 - y1;
    if (abs(dx) > abs(dy)) {
        while (static_cast<uint32_t>(x) != x2) {
            set_pixel(color, static_cast<uint32_t>(x), static_cast<uint32_t>(round(y)));
            y = y + dy / dx;
            x += 1;
        }
    } else {
        while (static_cast<uint32_t>(y) != y2) {
            set_pixel(color, static_cast<uint32_t>(round(x)), static_cast<uint32_t>(y));
            x = x + dx / dy;
            y += 1;
        }
    }
...
void ppm_t::show() const {
    for (auto pixel: m_image_buffer) {
        *m_file << pixel.getR() << ' ' << pixel.getG() << ' ' << pixel.getB() <<
                endl;
    }
}
